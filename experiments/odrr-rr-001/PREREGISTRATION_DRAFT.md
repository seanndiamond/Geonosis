...
### Power basis
N=60 provides approximately 80% power for a two-sided paired comparison at alpha=.05 for a standardized within-participant effect of approximately dz=0.40. The smallest effect of interest is separately defined as 0.75 points on the 0–10 ODRR scale.
...
- fixed exposure window per image;
- blank interval between orientations;
- ratings entered after each image;
- no researcher coaching during the session.

Exact timing values will be frozen before registry submission and before any confirmatory participant data are collected.
...
## 14. Confirmatory outcome classes
...
The 90% confidence interval lies entirely inside [-0.75, +0.75].

### CONTROL_ADVANTAGE / FALSIFIED_AT_TESTED_SCOPE
The 95% confidence interval lies entirely below 0.
...
- archive analysis code and synthetic validation outputs.
